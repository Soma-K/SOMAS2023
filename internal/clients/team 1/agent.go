...
func (bb *Biker1) nearestvLootColour() (uuid.UUID, float64) {
	currLocation := bb.GetLocation()
	shortestDist := math.MaxFloat64
...
func (bb *Biker1) GetAgentFromId(agentId uuid.UUID) obj.IBaseBiker {
	...
	return bb //WHAT TO RETURN IN GENERAL IF FOR SOME REASON INCORRECT AGENTID IS SENT???
}
...
//--------------------PSEUDO CODE FOR Direction choice with governance-------------------

func (bb *Biker1) DecideWeight() { //takse in proposed action as a parameter
...
func (bb *Biker1) DictateDirection() uuid.UUID {
	// only run for the dictator (server-side)
	return bb.proposeDirection()
	// just run contents of loot box nomination, i.e. nearest one etc
}
